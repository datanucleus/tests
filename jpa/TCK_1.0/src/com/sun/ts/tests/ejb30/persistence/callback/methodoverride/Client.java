  ...
  */
/*
 * @(#)Client.java	1.4 06/04/12
 */

package com.sun.ts.tests.ejb30.persistence.callback.methodoverride;
...
public class Client extends EntityCallbackClientBase 
	...
	getEntityTransaction().begin();
        ...
        List p = getEntityManager().createQuery(
                "Select DISTINCT Object(p) From Product p where p.id like '%Test%'")
                .setMaxResults(50)
                .getResultList();
		...
		getEntityTransaction().begin();
        	product = newProduct(id);
                ...
                String[] expected = new String[] {
                    LISTENER_A, LISTENER_B, LISTENER_C, PRODUCT
                };
                List actual = product.getPrePersistCalls();
        	Helper.compareResultList(expected, actual);
		getEntityTransaction().commit();	
	} catch (Exception e) {
          throw new Fault(e);
	} finally {
	...
	try {
		getEntityTransaction().begin();
        	...
        	getEntityManager().persist(order);
                ...
                String[] expected = new String[] {
                    LISTENER_A, LISTENER_B, LISTENER_C, ORDER
                };
                List actual = order.getPrePersistCalls();
        	Helper.compareResultList(expected, actual);
                
        	expected = new String[] {
                    LISTENER_A, LISTENER_B, LISTENER_C, LINE_ITEM
                };
...
//        	}
     ...
     * @assertion_ids:  PERSISTENCE:SPEC:694; PERSISTENCE:SPEC:704
     * @test_Strategy:
     */
